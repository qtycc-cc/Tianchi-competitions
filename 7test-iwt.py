...
from util import calculate_group_mi, result_beep, log
...
def train_model(X: pd.DataFrame, y: pd.Series, X_test: pd.DataFrame):
    ...
    categorical_features = ['is_weekend', 'month', 'day', 'hour', 'dayofyear', 'dayofweek']
    ...
    #         torch.tensor(X_num.to_numpy(), dtype=torch.float32, device=device),
    #         torch.tensor(y.to_numpy(), dtype=torch.float32, device=device),
    #         gidx,
    #         sgidx,
    #     )
    # print(f'Gmi = {gmi}')
    # print(f"X_num_list = {X_num.columns.to_list()}")
    ...
    return iwt.predict_proba(X_test)[:, 1]

@log(enable_file=True, file_path='7test-iwt.log')
@result_beep
def main():
...
if __name__ == '__main__':
    # import sys
    # sys.stdout = open(f'7test-iwt.txt', 'w', encoding='utf-8')
    main()